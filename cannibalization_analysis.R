...
proj_dir   <- "j:/JoshPomerantz/2026/concessions_promotions/Concessions"
...
to_date <- function(x) {
  ...
  if (inherits(x, "POSIXct")) return(as.Date(x))x
...
kpis <- read_excel(file.path(raw_dir, "braves_concessions_kpis_25_26.xlsx")) %>%
...
# valid games with the covariates used across the analysis
games <- game_info %>%
...
beer_relevant_tiers <- c(0, 1, 2, 3, 4, 5)   # may_in_the_a {2,3}, spring {5}, slide {0,1,3,4}
food_relevant_tiers <- c(0, 1, 3, 4, 5)      # spring {5}, slide {0,1,3,4}
...
promoted_beer <- "Miller High Life 12oz"      # normalized
...
# FIX: win_base_dates is now promo-specific -- candidate baseline dates are
# "this specific promo type is off", not "no promo at all is running".
win_base_dates <- function(promo_dates, promo_col) {
  nd <- games$saleAttribution_date[games[[promo_col]] == 0]
  nd_doy <- as.numeric(format(nd, "%j"))          # day-of-year for each candidate baseline date
  promo_doy <- as.numeric(format(promo_dates, "%j"))  # day-of-year for each promo date
...
# FIX: share_of baseline is now promo-specific
...
# =============================================================================
# FIX: restored the baseline clause (promo-specific), which had been dropped
# entirely -- this previously caused every call to return NULL.
...
# FIX: run_did baseline is now promo-specific
...
#     Assumes promotions add a flat X% to turnstile (input, not modeled here).
#     Applies that assumed lift to all "any concession promo" games, and
#     estimates the resulting concessions + retail revenue lift using each
#     game's own per-cap spending.
# =============================================================================

ASSUMED_TURNSTILE_LIFT <- 0.045   # <- the input assumption; change this to test other scenarios

# ---- Step 1: all promo games (beer or food promo), regardless of type ----
promo_games <- games %>%
  filter(beer_promo == 1 | food_promo == 1) %>%
  distinct(saleAttribution_date, turnstile)

nrow(promo_games)  # confirm this is 19

# ---- Step 2: total concessions revenue per game (food + beer combined) ----
concession_rev_for_lift <- read_excel(file.path(raw_dir, "braves_concessions_sales_25_26.xlsx")) %>%
  ...
  mutate(saleAttribution_date = if (is.character(date)) mdy(date) else as.Date(date)) %>%
  select(saleAttribution_date, revenue_total)

# ---- Step 3: pull retail sales, already provided ----
con <- dbConnect(
  bigquery(),
  project = "braves-data-prod",
  dataset = "Xenial_Lake",
  billing = "braves-data-prod"
)

retail_sales <- dbGetQuery(con, 'SELECT business_date
  ,count(distinct order_id) as num_orders 
  ,sum(net_sales) as net_sales
  ,sum(gross_sales) as gross_sales
  FROM `braves-data-prod.Xenial_Lake.Order` 
  WHERE order_state = "closed"
  and business_date not in ("2025-07-11","2025-07-12","2025-07-13","2025-07-14","2025-07-15")
  group by business_date') %>%
  mutate(business_date = as.Date(business_date)) %>%
  rename(saleAttribution_date = business_date)


# ---- Step 4: join everything onto promo_games ----
...
# Sanity check: any games missing revenue or retail data?
attendance_lift_table %>% summarise(
  missing_concessions = sum(is.na(revenue_total)),
  missing_retail = sum(is.na(retail_net_sales))
)

print(attendance_lift_table, n = 20)


# ---- Step 5: totals across all 19 games ----
...
print(attendance_lift_table_items, n = 20)
...
# ---- Per-game table ----
per_game_table <- attendance_lift_table %>%
  arrange(saleAttribution_date) %>%
  select(saleAttribution_date, projected_turnstile, actual_turnstile, added_fans,
         concessions_percap, retail_percap, concessions_lift, retail_lift) %>%
  mutate(total_lift = concessions_lift + retail_lift) %>%
  gt() %>%
  tab_header(title = "Concession Promotion Attendance & Revenue Lift, by Game",
             subtitle = paste0("Assumes a ", ASSUMED_TURNSTILE_LIFT * 100, "% attendance lift on promotion nights")) %>%
  cols_label(
    saleAttribution_date = "Date",
    projected_turnstile = "Projected Turnstile",
    actual_turnstile = "Actual Turnstile",
    added_fans = "Added Fans",
    concessions_percap = "Concessions Per Cap",
    retail_percap = "Retail Per Cap",
    concessions_lift = "Concessions Lift",
    retail_lift = "Retail Lift",
    total_lift = "Total Revenue Lift"
  ) %>%
  fmt_date(columns = saleAttribution_date, date_style = "m_day_year") %>%
  fmt_number(columns = c(projected_turnstile, actual_turnstile, added_fans), decimals = 0) %>%
  fmt_currency(columns = c(concessions_percap, retail_percap), decimals = 2) %>%
  fmt_currency(columns = c(concessions_lift, retail_lift, total_lift), decimals = 0) %>%
  tab_style(style = cell_text(weight = "bold"), locations = cells_column_labels()) %>%
  tab_style(style = cell_text(weight = "bold"), locations = cells_body(columns = total_lift))

per_game_table

# ---- Totals summary table ----
totals_table <- attendance_lift_totals %>%
  gt() %>%
  tab_header(title = "Total Attendance & Revenue Lift Across All Promotion Games",
             subtitle = paste0("19 promotion games, assuming a ", ASSUMED_TURNSTILE_LIFT * 100, "% attendance lift")) %>%
  cols_label(
    n_games = "Promo Games",
    total_actual_turnstile = "Total Actual Turnstile",
    total_projected_turnstile = "Total Projected Turnstile",
    total_added_fans = "Total Added Fans",
    total_concessions_lift = "Total Concessions Lift",
    total_retail_lift = "Total Retail Lift",
    total_revenue_lift = "Total Revenue Lift"
  ) %>%
  fmt_number(columns = c(n_games, total_actual_turnstile, total_projected_turnstile, total_added_fans), decimals = 0) %>%
  fmt_currency(columns = c(total_concessions_lift, total_retail_lift, total_revenue_lift), decimals = 0) %>%
  tab_style(style = cell_text(weight = "bold"), locations = cells_column_labels()) %>%
  tab_style(style = cell_text(weight = "bold", size = px(16)), locations = cells_body(columns = total_revenue_lift))

totals_table

gtsave(per_game_table, file.path(fig_dir, "attendance_lift_per_game_table.png"),
       vwidth = 1400, vheight = 900)
gtsave(totals_table, file.path(fig_dir, "attendance_lift_totals_table.png"))
