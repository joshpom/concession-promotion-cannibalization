...
concession_rev <- read.csv(file.path(raw_dir, "braves_concessions_sales_25_26.csv"), check.names = FALSE) %>%
...
concession_rev_2024 <- read.csv(file.path(raw_dir, "braves_concessions_sales_24.csv"), check.names = FALSE) %>%
  mutate(across(c(revenue_fandb, revenue_food, revenue_bar, revenue_other,
                   revenue_retail, revenue_total), ~ parse_number(as.character(.)))) %>%
  mutate(date = as.Date(date))
...
# ---- Load & clean KPI data ----
concession_kpis <- read.csv(file.path(raw_dir, "braves_concessions_kpis_25_26.csv"), check.names = FALSE) %>%
  mutate(
    saleAttribution_date = as.Date(saleAttribution_date),
...
concession_kpis_2024 <- read.csv(file.path(raw_dir, "braves_concessions_kpis_24.csv"), check.names = FALSE)  %>%
  mutate(
    saleAttribution_date = as.Date(saleAttribution_date),
    across(c(transactions, total, quantity, turnstile, trans_per_turnstile,
             items_per_trans, spend_per_trans, percap), ~ parse_number(as.character(.))),
...
ggsave(
  filename = file.path(figures_dir, "turnstile_by_day_promo.png"),
  plot = turnstile_plot,
  width = 8,
  height = 5,
  dpi = 300
...
ggsave(
  filename = file.path(figures_dir, "percap_by_day_promo.png"),
  plot = percap_plot,
  width = 8,
  height = 5,
  dpi = 300
...
ggsave(
  filename = file.path(figures_dir, "items_per_turnstile_by_day_promo.png"),
...
ggsave(
  filename = file.path(figures_dir, "trans_per_turnstile_by_day_promo.png"),
...
ggsave(
  filename = file.path(figures_dir, "revenue_by_day_promo.png"),
  plot = revenue_plot,
  width = 8,
  height = 5,
  dpi = 300
...
schedule_2024 <- read.csv(file.path(raw_dir, "2024_Schedule_info.csv")) %>% mutate(Date = mdy(Date))
schedule_2025 <- read.csv(file.path(raw_dir, "2025_Schedule_info.csv")) %>% mutate(Date = mdy(Date))
schedule_2026 <- read.csv(file.path(raw_dir, "2026_Schedule_info.csv")) %>% mutate(Date = mdy(Date))
...
# Adding Ticket Sales (from pre-exported CSV instead of BigQuery)

ticket_sales <- read.csv(file.path(raw_dir, "ticket_sales_by_game.csv"), check.names = FALSE) %>%
  mutate(saleAttribution_date = as.Date(saleAttribution_date))

ticket_sales_by_game <- ticket_sales
...
items_2025 <- read.csv(file.path(raw_dir, "concessions_2025_item_level.csv"))
items_2026 <- read.csv(file.path(raw_dir, "concessions_2026_item_level.csv"))
stands_2025 <- read.csv(file.path(raw_dir, "concessions_2025_stand_level.csv")) %>%
  mutate(saleAttribution_date = as.Date(saleAttribution_date))
stands_2026 <- read.csv(file.path(raw_dir, "concessions_2026_stand_level.csv")) %>%
  mutate(saleAttribution_date = as.Date(saleAttribution_date))
...
tiers_2025 <- read.csv(file.path(raw_dir, "2025_Schedule_info.csv")) %>%
  select(Date, Tier) %>%
  mutate(Date = as.Date(Date, format = "%m/%d/%Y"))

tiers_2026 <- read.csv(file.path(raw_dir, "2026_Schedule_info.csv")) %>%
  select(Date, Tier) %>%
  mutate(Date = as.Date(Date, format = "%m/%d/%Y"))
...
showrate_overall <- read.csv(file.path(raw_dir, "showrates_overall.csv"), check.names = FALSE) %>%
...
# ---- Output folder ----
item_output_dir <- file.path(figures_dir, "Item Performance Promo vs Nonpromo")
if (!dir.exists(item_output_dir)) dir.create(item_output_dir, recursive = TRUE)
...
analyze_and_save_item <- function(item_name, promo_type) {
  ...
  ggsave(file.path(item_output_dir, paste0(safe_name, "_quantity.png")),
  ...
  ggsave(file.path(item_output_dir, paste0(safe_name, "_quantity_per_turnstile.png")),
  ...
  list(
    ...
    ttest_pt = ttest_pt,
    wilcox_pt = wilcox_pt,
    plot_qty_path = file.path(item_output_dir, paste0(safe_name, "_quantity.png")),
    plot_pt_path = file.path(item_output_dir, paste0(safe_name, "_quantity_per_turnstile.png"))
  )
}
...
saveRDS(all_item_results, file.path(item_output_dir, "all_item_results.rds"))
